...
| **a2bot_extras** | Optional add-ons: **two separate** web interfaces (a FastAPI telemetry/WiFi-setup page and a React control interface reached via rosbridge — see [Dashboard & rosbridge](dashboard-and-rosbridge.md)), WiFi provisioning, gesture control. Nothing in `a2bot_driver`, `a2bot_description`, `a2bot_bringup`, or `a2bot_navigation` depends on this one — though `a2bot_extras` itself does depend on `a2bot_service`, since both web interfaces call straight into its movement services. |
