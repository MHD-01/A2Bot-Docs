...
!!! pi "🤖 Pi"
    ...
    This FastAPI page (port `8888`) is **not** the same thing as the React-based control interface on port `7777` (`a2bot-webui.service`) — the two are separate programs, reached differently, and easy to conflate. See [Dashboard & rosbridge](../part2/dashboard-and-rosbridge.md) for the full distinction and how the control interface talks to ROS 2 over rosbridge instead of directly.
