...
- Both machines must share the same **`ROS_DOMAIN_ID`** (this robot's own number, `X` — see [Setup 1](setup-1-raspberry-pi.md#8-set-the-ros-domain-id) or [Setup 2](setup-2-laptop.md#4-match-the-ros-domain-id)), so ROS 2 doesn't lump you in with every other domain-0 ROS system on the same network.
